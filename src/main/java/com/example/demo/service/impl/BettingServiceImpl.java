...
public class BettingServiceImpl implements BettingService{
	...
	@Transactional
	@Override
	public JsonDto doBettingStr(Long lotteryNumId, Long lotteryTypeId, Long ruleId, String bettingInfoStr,Long playerId) {
		...
		try {
			...
			if(null != bettingInfoList ) {
				...
				//3）判断余额是否可购买，
				double balanceVal = 0;
				Balance balance = balanceService.getBalanceByPlayerId(playerId);
				if(null != balance) {
				...
				balanceRecord.setPlayerId(playerId);				
	...
	@Transactional
	@Override
	public JsonDto doJoinBuyStr(Long lotteryNumId, Long lotteryTypeId, Long ruleId, String bettingInfoStr,
			String joinBuyInfoStr, Long playerId) {
		List<BettingInfoVo> bettingInfoList = null;
		JsonDto json = new JsonDto();
		try {
			//1)判断请求参数是否合法
		...
		try {
			...
			if(null != bettingInfoList && null != joinBuyInfo) {
				...
				joinBuyDetail.setPlayerId(playerId);
